// Entry point for the Microsoft OAuth application
...
async function main() {
  try {
    ...
    try {
      console.log('Trying to initialize Redis...');
      ...
      console.warn('Redis connection failed, using memory session store instead:', redisError.message);
      // Continue with memory store (the default)
    }
    ...
    // Dynamic import to avoid issues with ESM loading
    const authModule = await import('./routes/auth.js');
    const authRouter = authModule.default;
    const { initializeMicrosoftClient } = authModule;
    
    const authMiddlewareModule = await import('./middleware/auth.js');
    const { requireAuth, setUserLocals, checkTokenValidity } = authMiddlewareModule;
    ...
    // Start server
    console.log('Initializing Microsoft client...');
    try {
      await initializeMicrosoftClient();
      console.log('Microsoft client initialized');
    } catch (msError) {
      console.warn('Microsoft client initialization failed:', msError.message);
      console.warn('Some OAuth functionality may be limited');
    }
